...
system_instruction = '''Task: Text Span Identification for Arguments
...
def formatting_prompts_func(example):
    ...
    input1 = example['context']
    input2 = example['question']
...
dataset = load_dataset("csv", data_files={"train": "task2.csv"}, delimiter='\t')
...
for fold, (train_idx, test_idx) in enumerate(kf.split(dataset["train"])):
    ...
    trainer = SFTTrainer(
        ...
        args = TrainingArguments(
            ...
            seed = 3407,
            output_dir = "outputs_task2_"+ str(fold)
        ),
    ...
    with jl.open('output_lora_task2_'+str(fold) + '.jsonl','w') as w:
